...
1. **Split Strategy**:
   ...
   - Calculates **Class Weights** based on pixel frequency in the training fold to handle class imbalance. 
     - Utilizes **`CLASS_WEIGHT_MODE`** to determine the smoothing strategy (`inverse`, `median`, `sqrt`, `log`). `log` provides the most extreme dampening for severely imbalanced datasets with noisy minority classes.
     - Utilizes **`CLASS_WEIGHT_CLIP`** to cap the maximum weight assigned to extreme minority classes, preventing the model from becoming hypersensitive and outputting false positives for those classes.
     - If the model is still predicting too many false positives for the extreme minority class, decrease `CLASS_WEIGHT_CLIP` in config/config.yaml (e.g., to 3.0 or 2.0), or use `CLASS_WEIGHT_MODE: "log"` or `"sqrt"`.
     - If the model completely ignores the extreme minority class and you want it to learn it, increase `CLASS_WEIGHT_CLIP` in config/config.yaml (e.g., to 10.0) or use `CLASS_WEIGHT_MODE: "inverse"`.
2. **Optimization**:
